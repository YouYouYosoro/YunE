<template>
  <div>
    <div class="positionManaTool">
      ...
      <el-input v-model="role.nameZh" placeholder="请输入角色中文名" size="small"
                @keydown.enter.native="doAddRole"></el-input>
      <el-button icon="el-icon-plus" size="mini" type="primary" @click="doAddRole">添加角色</el-button>
    </div>
    <!-- 手风琴 -->
        ...
        <el-collapse-item v-for="(r,index) in roles" :key="index" :name="r.id" :title="r.nameZh">
          <el-card class="box-card">
            <div slot="header" class="clearfix">
              <span>可访问资源</span>
              <el-button icon="el-icon-delete" style="float: right;padding: 3px 0;color: #f41f0a" type="text"
                         @click="doDeleteRole(r)">
              </el-button>
            </div>
            <div>
              <el-tree :key="index" ref="treeRef" :data="allMenus" :default-checked-keys="selectedMenus"
                       :props="defaultProps"
